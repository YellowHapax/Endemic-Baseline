"""Re-zeroing Protocol ΓÇö Scaffolded First Contact with the Horizon
...
excursions into the Horizon ΓÇö the set of states in H_accessible
that are outside H_agent.

Protocol requirements derived from Paper 7 theory:
    1. Controlled novelty:   |I(t) - B(t)| Γëê theta_h + epsilon
...
# ΓöÇΓöÇ Metadata ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

PAPER = 7
PAPER_TITLE = "The Endemic Baseline"
LAB_TITLE = "Phenomenon P7b: Re-zeroing Protocol ΓÇö Scaffolded Horizon Contact"
...
THESIS = (
    ...
    "Withdrawal (protocol terminated at the Identity Dissolution zone ΓÇö the "
...
# ΓöÇΓöÇ Protocol helpers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def _kappa_update(kappa: float, *, low_novelty_bond: float, dt: float = 1.0,
    ...
    d╬║/dt = ╬▒(1 - N_bond) - ╬▓┬╖╬║   (from Paper 4 coupling mechanics)
    """
...
# ΓöÇΓöÇ Simulation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def run(
    *,
    ...
    Strategy C ΓÇö Withdrawal:
        Identical to Re-zeroing until `withdrawal_step`, then inputs stop.
        The agent is in the Identity Dissolution zone ΓÇö baseline has migrated
        ...
        for t in range(steps):
            ...
            if strat == "flood":
                ...
                if t < kappa_build_steps:
                    ...
                    # Phase 2: micro-excursion ΓÇö input just over theta_h above B
                    I = b + theta_h + excursion_epsilon
                    phase = "excursion"
                else:
                    # Kappa not yet sufficient ΓÇö extend bonding
...
# ΓöÇΓöÇ Plotting ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def plot(results=None, **kw):
    ...
    # ΓöÇΓöÇ (0,0) Baseline trajectories ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ax = axes[0][0]
    ...
    ax.set(xlabel="Time", ylabel="Baseline B(t)",
           title="Baseline Trajectories ΓÇö Three Strategies")
    ax.legend(fontsize=7)

    # ΓöÇΓöÇ (0,1) Kappa trajectories ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ax = axes[0][1]
    for name, s in series.items():
        ts = [d["time"] for d in s]
    ...
    ax.axhline(params["kappa_threshold"], color="purple", linestyle="--",
               linewidth=1.5, alpha=0.7, label=f"╬║ threshold = {params['kappa_threshold']}")
    ax.set(xlabel="Time", ylabel="╬║ (relational coupling)",
           title="Kappa Trajectories ΓÇö Coupling Builds Before Excursion")
    ax.legend(fontsize=7)

    # ΓöÇΓöÇ (1,0) B-shift comparison bars ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ax = axes[1][0]
    ...
    ax.set(ylabel="Total B shift from B_storm",
           title="Net Baseline Migration ΓÇö Final Outcome")
    for bar, val in zip(bars, shifts):
        ax.text(bar.get_x() + bar.get_width() / 2,
                val + (0.002 if val >= 0 else -0.01),
                f"{val:+.4f}", ha="center", va="bottom", fontsize=9)

    # ΓöÇΓöÇ (1,1) Phase annotation for Re-zeroing ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ax = axes[1][1]
    ...
    ax_k.plot(ts, ks, color="purple", linewidth=1.2, linestyle="--",
              alpha=0.6, label="╬║")
    ax_k.set_ylabel("╬║", color="purple", fontsize=9)
    ax.axhline(params["b0_storm"], color="grey", linestyle=":", linewidth=1, alpha=0.6)

    # Legend for phases
    legend_patches = [
        mpatches.Patch(color=phase_colors["kappa_build"], alpha=0.5, label="╬║-build"),
