"""Endemic Baseline ΓÇö Calibration Failure and Healthy-Input Rejection
...
MBD predicts three things in the endemic case:
    ...
    3. IDENTITY DISRUPTION WITHOUT LANDING ZONE: Even when the Emergent
       ...
       B_storm and into H_accessible \\ H_agent ΓÇö uncharted territory
...
because recovery has a *target* ΓÇö the prior healthy encoding.
"""
...
LAB_TITLE = "Phenomenon P7a: Endemic Baseline ΓÇö Healthy-Input Rejection"
...
# ΓöÇΓöÇ Core dataclass ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def run(
    *,
    ...
    The key experimental distinction:
      ...
      - Storm-born agent has no B_ref ΓÇö the endemic flag is set.
    ...
    agents = [
        ...
        Agent(
            ...
            b_ref_healthy=None,   # ΓåÉ The endemic flag. No prior healthy encoding.
        ),
    ]
    ...
    for t in range(steps):
        ...
        for ag in agents:
            ...
            # High kappa ΓåÆ input is integrated at face value (╬╗ unchanged).
            # Low kappa + high novelty ΓåÆ partial rejection; effective_╬╗ reduced.
            ...
            if ag.endemic and I != 0.0:
                ...
                # Destabilization Γê¥ distance from storm ├ù (1 - kappa).
...
def plot(results=None, **kw):
    ...
    # ΓöÇΓöÇ (0,1) Peak novelty comparison ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ax = axes[0][1]
    ...
    ax.axhline(params["theta_h"], color="red", linestyle="--",
               alpha=0.6, label=f"╬╕_h = {params['theta_h']}")
    ax.set(ylabel="Peak Novelty", title="Peak Novelty ΓÇö Same Input, Different Reference")
    ax.legend(fontsize=8)
    for bar, val in zip(bars, novelties):
        ax.text(bar.get_x() + bar.get_width() / 2, val + 0.01,
                f"{val:.2f}", ha="center", va="bottom", fontsize=9)

    # ΓöÇΓöÇ (1,0) Destabilization index (endemic agent only) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ax = axes[1][0]
    for name, s in series.items():
    ...
    # ΓöÇΓöÇ (1,1) Rejection rate table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ax = axes[1][1]
    ax.axis("off")
    table_data = [
        ["Agent", "BΓéÇ", "B final", "Shift", "Rejection rate"],
    ]
    for c in comp:
        table_data.append([
            short_names[c["agent"]],
            ...
            f"{c['b_final']:.3f}",
            f"{c['b_shift']:+.3f}",
            f"{c['pct_rejected']*100:.0f}%{'  ΓåÉ endemic' if c['endemic'] else ''}",
        ])
    ...
    tbl.set_fontsize(9)
    tbl.scale(1.2, 1.6)
    ax.set_title("Integration Outcomes ΓÇö Identical Input, Structural Difference",
                 fontsize=10, pad=12)
...
if __name__ == "__main__":
    ...
    for c in results["comparison"]:
        flag = " ΓåÉ ENDEMIC" if c["endemic"] else ""
